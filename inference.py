...
def load_frame(frame_path):
    return (torchvision.io.read_image(str(frame_path)) / 255.).unsqueeze(0).to(device)


def interpolate_frame_sequence(frames_dir):
    frames_dir = Path(frames_dir)
    frame_paths = sorted(frames_dir.glob("frame*.0.png"))
    if len(frame_paths) < 2:
        raise RuntimeError(f"Expected at least 2 input frames matching frame*.0.png in {frames_dir}")

    total_pairs = len(frame_paths) - 1
    for index, (frame_0_path, frame_1_path) in enumerate(zip(frame_paths, frame_paths[1:]), start=1):
        with torch.no_grad():
            frame_0 = load_frame(frame_0_path)
            frame_1 = load_frame(frame_1_path)
            interpolated_frame = interpolate(frame_0, frame_1)

        interpolated_path = frame_0_path.with_name(frame_0_path.name.replace(".0.", ".5."))
        torchvision.utils.save_image(interpolated_frame, str(interpolated_path))
        del frame_0, frame_1, interpolated_frame
        torch.cuda.empty_cache()

        if index % 50 == 0 or index == total_pairs:
            print(f"Processed {index}/{total_pairs} frame pairs...")
...
if frames_dir:
    print(f"Interpolating frame sequence in {frames_dir} ...")
    interpolate_frame_sequence(frames_dir)
    print(f"Saved interpolated frames in {frames_dir}.")
elif video_path:
